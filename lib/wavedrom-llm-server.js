...
function write_log(filename, log_obj) { 
  // Log the chat session
  fs.writeFile(filename, JSON.stringify(log_obj), (err) => {
    if (err) {
...
function create_new_chat(chat_id, ip) { 
    ...
    // Log the chat session
    write_log(filename,new_chat_obj);
...
app.put('/chat/:chat_id', (req, res) => {    
  ...
  if (status_code == 200) { 
    ...
    // Get logs
    let log_filename = chatSessions[chat_id]['filename'];
    let chat_history_text = fs.readFileSync(log_filename);
    ...
    llm_resp.then(data => {
      ...
      // Write the log
      write_log(log_filename,chat_history);
