...
PUT /chat/<chat_id>?prompt=<prompt>[&json=<json_text>&no_log=true] 
...
let yaml_file = './llm/system_instructions.yaml';

function read_model_information(yaml_file) { 

  // Read the version
  const sys_instr = yaml.load(fs.readFileSync(yaml_file, 'utf-8'));
  console.log(sys_instr['version']);
  console.log(sys_instr['model']);

  let version = sys_instr['model'] + "_WavedromLLM-" + sys_instr['version']; 
  console.log('Setting version ' + version);

  const model = genAI.getGenerativeModel({
    model: sys_instr['model'],
    systemInstruction: sys_instr['instructions']
  });
  ...
  return [version, model, generationConfig];
...
function create_new_chat(chat_id, ip) { 
    ...
    let [version, model, generationConfig] = read_model_information(yaml_file);
...
function clean_up_repeated(resp_text) { 

  // setup initial string
  let new_string = resp_text;


  // Search for all the wave sections
  let search_pos = 0;
  let wave_pos = 0;
  let wave_indices = new Array();
  wave_pos = new_string.indexOf('"wave"', search_pos);
  //console.log('first wave_pos = '+ wave_pos);
  while (wave_pos >= 0) { 
    const start_pos = new_string.indexOf('"', wave_pos+6)+1;
    const end_pos = new_string.indexOf('"', start_pos+1);
    const wave_range = new Array(start_pos, end_pos);
    wave_indices.push(wave_range);
  ...
  // replace repeated 0s or 1s
  for (let loop1 = 0; loop1 < wave_indices.length; loop1 += 1) { 
    let replace_char = '';
    let replace_dot = false;
    for (let char_index = wave_indices[loop1][0]; char_index < wave_indices[loop1][1]; char_index += 1) { 

      if (replace_char == '') {
        if ((new_string[char_index] == '0') || (new_string[char_index] == '1')) {
          replace_char = new_string[char_index];
        }
        replace_dot = false;
      } else { 
        if (new_string[char_index] == replace_char) {
          replace_dot = true;
        } else { 
          if ((new_string[char_index] == '0') || (new_string[char_index] == '1')) {
            replace_char = new_string[char_index];
      ...
      // create a new string
      if (replace_dot) { 
        console.log('old_string ' + new_string);
        new_string = new_string.slice(0, char_index) + '.' + new_string.slice(char_index+1)
...
app.post('/new_chat', (req, res) => {    
  ...
  if (check_api_key(req.body)) {
    ...
    try {
      ...
      resp_object['version'] = chatSessions[chat_id]['version'];
  ...
  if (chatSessions.hasOwnProperty(chat_id)) {
    ...
    if (status_code == 200) { 
      ...
      try { 
        ...
        llm_resp.then(data => {
          const resp_text = clean_up_repeated(data);
          console.log("llm_resp " + resp_text);
          resp_object['json'] = resp_text;
    
          const end_time = process.hrtime(start_time);
          let exec_time = parseInt(end_time[0]/1.0e-3 + end_time[1]/1.0e6, 10);

          let exec_text = 'LLM model took ' + exec_time + "ms to complete with version " + chatSessions[chat_id]['version'];

          console.log(exec_text)
          resp_object['message'] += exec_text;

          // Update website
          chatSessions[chat_id]['json'] = resp_text;
          interaction['model'] = resp_text;
          interaction['timestamp'] = Date.now();
          interaction['time'] = exec_time;
          interaction['version'] = chatSessions[chat_id]['version'];
    
          // Write the log
          if (!(req.body.hasOwnProperty('no_log'))) {
  ...
  if (chatSessions.hasOwnProperty(chat_id)) {
    ...
    if (req.body.hasOwnProperty('json')) { 
      ...
      try {
        ...
        resp_object['svg'] = 'ERROR in create_svg: ' + error;
