...
function get_chat_history(filename) { 
  ...
  try { 
    chat_history_text = fs.readFileSync(fs_path + filename);
    chat_history = JSON.parse(chat_history_text);
  } catch (err) {
    console.log("ERROR in get_chat_history()");
    console.log("text: " + chat_history_text);
    ...
    if (status_code == 200) { 
      ...
      try { 
        ...
        llm_resp.then(data => {
          ...
          resp_object['message'] += 'Model took ' + exec_time + "ms to complete.";
